...
Author: Barbara Barcelo Llull, IMEDEA, May 2018
...
def compute_OI_2d(lon,lat,U,loni,lati,lx,ly,ang,eps,T_mean) :
    ...
    # new grid in km respecte al punt (lon, lat) mÌnim de la malla original
    ...
    # Ho necessitare mes endavant pels reshapes
    nxg        = len(loni)
    nyg        = len(lati) 
    ...
    # Calcul matriu de correlacions entre totes les estacions
    ...
    # Calcula correlacions entre els punts de malla i les estacions
    ...
    for i in np.arange(nlatlon):
        for j in np.arange(nxg*nyg):
             ...
             # I supose that YYin(j) does the job XXin.flatten()[j]
